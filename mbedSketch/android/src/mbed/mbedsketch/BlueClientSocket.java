...
public class BlueClientSocket implements Runnable{
	...
	public void run() {
		...
		try {
			...
			tmp = remDevice.createRfcommSocketToServiceRecord(MbedADKSketchActivity.myUUID);
		} catch (IOException e) {
	...
	public void manageBlueClient(){
		...
		try {
			MbedADKSketchActivity.output = blueClient.getOutputStream();
		} catch (IOException e) {
			Log.i("BlueLog", "Client: OutputStream did NOT initialize...");
			e.printStackTrace();
		}
		Log.i("BlueLog", "Client: Input & Output are initialised...");
		Thread socketRead2 = new Thread(new SocketRead(input, MbedADKSketchActivity.output));
		socketRead2.start();
